...
public class QuickSortTests implements SortAlgorithmListener {
    private int[] input;

    @Test
    void testQuickSortSingleThread() {
        CountDownLatch latch = new CountDownLatch(2);

        int[] values = ArrayUtils.generateFixedArrayTen();

        SortExecutor sortExecutor = new SortExecutor(this);
        sortExecutor.setSelectedAlgorithmType(SortAlgorithmType.QUICK_SORT);
